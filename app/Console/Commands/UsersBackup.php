...
class UsersBackup extends Command
{
    ...
    public function handle()
    {
         ...
         */
        $filepath = $disk->path("users/$filename");

        try {
            /**
             * Recupero tutti i dati dalla tabella del database
             */
            $todos = User::orderBy($this->option('orderBy'), $this->option('orderDesc') ? 'desc' : 'asc')
                ->get();
            ...
            $this->info("La tabella degli utenti è stata esportata in «{$filepath}»");
